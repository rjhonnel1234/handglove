...
(new WOW).init();var RESET_PASSWORD={init:function(){$("#reg-form").validate({rules:{password:{minlength:8},confirm:{equalTo:"#password-2"}},submitHandler:function(e){return ajaxRequest({url:GLOBAL_BASE_URL+"login/change-password",type:"POST",data:$(e).serialize(),callback:function(e,r){r.success?($("#reg-form").remove(),$("#reg-basic").html(r.message)):$("#errorMsg").html('<div class="alert alert-danger" id="errorMsg">'+r.message+"</div>")}}),!1}}),$("#reg-form").submit(function(e){return e.preventDefault(),!1})}};$(document).ready(RESET_PASSWORD.init);