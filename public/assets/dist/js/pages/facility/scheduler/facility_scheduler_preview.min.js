...
$(document).ready(function(){$(document).on("click",".remove-staff-row",function(e){e.preventDefault();e=$(this).closest(".staff-row");let t=e.closest(".staff-assignment-list-preview");e.css("background-color","#fff5f5").fadeOut(300,function(){$(this).remove(),0===t.find(".staff-row").length&&0===t.find(".empty-shift-notice").length&&t.append('<div class="empty-shift-notice m-auto x-small text-muted italic py-4">Unassigned</div>')})}),$(document).on("click",".remove-unit-block",function(e){e.preventDefault(),confirm("Are you sure you want to remove this entire unit from the preview?")&&$(this).closest(".unit-block").css("border-color","#feb2b2").fadeOut(400,function(){$(this).remove(),0===$("#units-schedule-container .unit-block").length&&$("#units-schedule-container").append('<div class="alert alert-warning text-center py-5">All schedule data has been removed.</div>')})}),$("#convert_to_schedule_btn").on("click",function(e){e.preventDefault();let t=$(this);var e=$('input[name="upload_id"]').val(),a=$('input[name="date"]').val();let s=[];$(".unit-block").each(function(){let n=$(this);var e=n.data("unit-name");let i={};n.find("td[data-shift]").each(function(){var e=$(this).data("shift");let t=[];var a=n.find(`input.shift-slots-preview[data-shift-key="${e}"]`).val()||1;$(this).find(".staff-row").each(function(){var e=$(this);t.push({name:e.data("name"),position:e.data("position"),other:e.data("other")})}),i[e]={slots:a,staff:t}}),s.push({name:e,shifts:i})}),0===s.length?alert("No unit data found to convert."):confirm("This will convert the extraction into an official schedule. New units and staff will be created automatically. Proceed?")&&(t.prop("disabled",!0).html('<i class="fa fa-spinner fa-spin mr-2"></i> Converting...'),$.ajax({url:BASE_URL+"facility/schedules/save_manual",type:"POST",data:{upload_id:e,date:a,converted_data:JSON.stringify(s),is_conversion:1},dataType:"json",success:function(e){e.success?(alert(e.message||"Schedule converted successfully!"),window.location.href=BASE_URL+"facility"):(alert("Error: "+(e.message||"Unknown error occurred.")),t.prop("disabled",!1).html('<i class="fa fa-magic mr-2"></i> Convert to schedules'))},error:function(){alert("An error occurred during conversion. Please try again."),t.prop("disabled",!1).html('<i class="fa fa-magic mr-2"></i> Convert to schedules')}}))})});