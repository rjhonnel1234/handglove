                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                                ...
                                <?php foreach ($units as $unit): ?>
                                    <div class="unit-schedule-section mb-5" data-unit-id="<?= $unit['id'] ?>">
                                        <div class="unit-header mb-3 border-bottom pb-2">
                                            <h3 class="mb-0 text-primary"><i class="fa fa-hospital mr-2"></i>
                                                <?= $unit['name'] ?></h3>
                                        </div>
                                                    ...
                                                    <div class="card shift-card shadow-sm border-light h-100">
                                                        <div class="card-header bg-light border-bottom-0 pb-0">
                                                            <div class="shift-header-content p-2">
                                                                <h6 class="mb-0 text-center font-weight-bold"><?= $name ?> Shift
                                                                    <span class="badge badge-pill badge-info shift-slots-display" data-unit-id="<?= $unit['id'] ?>" data-shift="<?= strtolower($name) ?>"></span>
                                                                </h6>
                                                                <small
                                                                    class="text-muted d-block text-center"><?= $time ?></small>
                                                            </div>
                                                        </div>
                                                        <div class="card-body p-2">
                                                            <div class="staff-assignment-list"
                                                                id="list-unit-<?= $unit['id'] ?>-<?= strtolower($name) ?>">
                                                                <div class="empty-shift-notice text-muted italic p-2">No staff
                                                                    assigned.</div>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4 border-top pt-4">
                                <a href="<?= base_url('facility/') ?>" class="btn btn-warning"><i
                                        class="fa fa-arrow-left mr-1"></i> Back to Calendar</a>
                                <div>
                                    ...
                                    <?php endif; ?>
                                    <?php if ($scheduleUpload['status'] == 10 && $scheduleUpload['schedule_date'] >= date('Y-m-d')) { ?>
                                        <a href="<?= base_url('facility/schedules/add?date=' . $selectedDate) ?>"
                                            class="btn btn-success"><i class="fa fa-edit mr-1"></i> Edit Schedule</a>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
